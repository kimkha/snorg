<?php
	 ...
	 */
	function list_fr_for_user($relationship_guid, $is_inverse = true) {	
		$relationship = 'eventrequest';
		$inverse_relationship = $is_inverse;
	//	$type = "user";
		$subtype = "";
		$owner_guid = 0;
		$limit = 10;
	...
	function fr_view_entity_list($entities, $count, $offset, $limit, $fullview = true, $viewtypetoggle = true, $pagination = true) {
		...
		$html = elgg_view('eventrequest/entity_list',array(
												'entities' => $entities,
												'count' => $count,
												'offset' => $offset,
												'limit' => $limit,
	...
	$area1 = elgg_view_title(" Event Invitation");

	$area2 = list_fr_for_user($_SESSION['user']->guid);
		
	$body = elgg_view_layout('two_column_left_sidebar', '', $area1 . $area2);
	
	echo page_draw(" Event Invitation",$body);
